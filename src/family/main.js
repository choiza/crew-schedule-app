...
(function () {
  ...
  /** 고치는 줄 아래의 설명: 뜻, 어디서 온 뜻인지, 스케줄에 며칠 나왔는지 */
  function codeNote(code, word) {
    var days = codeDays(code);
    var source = SOURCE_TEXT[plan.wordSource(code, categoryOf(code), db.words)];
    var parts = [word.long, source, days ? days + '일' : ''].filter(Boolean);
    return '<small class="code-note">' + esc(parts.join(', ')) + '</small>';
  }
